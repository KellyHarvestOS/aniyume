...
export default function AccountSecuritySettings({ email, onEmailChange }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (!nextEmail.trim() || !currentPassword) {
      setMessage({ type: 'error', text: t('account.errFill') });
      return;
    }

    if (password && password !== passwordConfirmation) {
      setMessage({ type: 'error', text: t('account.errMismatch') });
      return;
    }
    ...
    if (!payload.email && !payload.password) {
      setMessage({ type: 'error', text: t('account.errNothing') });
      return;
    }
    ...
    try {
      ...
      setMessage({ type: 'error', text: error instanceof Error ? error.message : t('account.updateFailed') });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <section className="md:col-span-2 space-y-5 rounded-xl border border-slate-200 bg-slate-50/60 p-6 dark:border-white/10 dark:bg-[#161616]">
      <div>
        ...
        <p className="mt-1 text-xs font-semibold text-slate-400">{t('account.desc')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <label className="space-y-2">
          <span className="text-[11px] font-black uppercase tracking-[0.15em] text-slate-700 dark:text-gray-200">{t('account.newPassword')}</span>
          <input type="password" value={password} onChange={(event) => setPassword(event.target.value)} className={inputClass} minLength={8} autoComplete="new-password" placeholder={t('account.newPasswordPlaceholder')} />
        </label>

        <label className="space-y-2">
          <span className="text-[11px] font-black uppercase tracking-[0.15em] text-slate-700 dark:text-gray-200">{t('account.confirmPassword')}</span>
