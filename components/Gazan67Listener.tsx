...
const YOUTUBE_ID = 'yo5CILhgvT4';

export default function Gazan67Listener() {
  ...
  useEffect(() => {
    ...
    // Публичный канал — авторизация не нужна, ловят все посетители.
    echo.channel('gazan').listen('.gazan.67', (data: { active: boolean }) => {
  ...
  return (
    <>
      {/* Скрытый плеер с музыкой Газана. autoplay=1; браузер может потребовать
          предварительного клика по странице, чтобы разрешить звук. */}
      <iframe
        title="gazan-67-audio"
      ...
      {/* Полупрозрачные «67» по всему фону */}
      <div
        aria-hidden
        style={{
          position: 'fixed',
          ...
          return (
            <span
              ...
              style={{
                ...
                animation: `gazan67float ${duration}s ease-in-out ${delay}s infinite`,
                userSelect: 'none',
              }}
            >
              67
