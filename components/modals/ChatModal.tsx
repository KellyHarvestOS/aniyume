...
export default function ChatModal({ onClose }: { onClose: () => void }) {
    ...
    return (
        ...
        >
            <div className="bg-linear-to-r from-[#2EC4B6] to-[#28b1a4] p-4 flex items-center justify-between cursor-grab active:cursor-grabbing shadow-md">
                    ...
                    >
                        <div className={`max-w-[85%] p-3 rounded-2xl text-[13px] leading-relaxed shadow-sm ${msg.role === 'user'
                            ...
                            <ReactMarkdown
                                ...
                                components={{
                                    strong: ({ node, ...props }) => <strong className="font-black text-[#2EC4B6] dark:text-[#21D0B8]" {...props} />,
                                    ul: ({ node, ...props }) => <ul className="list-disc ml-4 my-2" {...props} />,
                                    ol: ({ node, ...props }) => <ol className="list-decimal ml-4 my-2" {...props} />,
                                    a: ({ node, ...props }) => <a className="text-[#2EC4B6] underline hover:no-underline" target="_blank" {...props} />,
                    ...
                    <div className="flex justify-start">
                        <div className="bg-white dark:bg-[#1a1a1a] p-3 rounded-2xl rounded-tl-none border border-slate-200 dark:border-white/5 shadow-sm">
                            <span className="flex gap-1">
                                <span className="w-1.5 h-1.5 bg-[#2EC4B6] rounded-full animate-bounce"></span>
                                <span className="w-1.5 h-1.5 bg-[#2EC4B6] rounded-full animate-bounce [animation-delay:0.2s]"></span>
                                <span className="w-1.5 h-1.5 bg-[#2EC4B6] rounded-full animate-bounce [animation-delay:0.4s]"></span>
                            </span>
                        </div>
                    </div>
                )}
            ...
            <div className="p-4 bg-white dark:bg-[#0f0f0f] border-t border-slate-200 dark:border-white/5">
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                        {['Расписание', 'Проблемы с плеером', 'Помощь'].map((text) => (
                            <button
                                ...
                                className="text-[11px] font-bold py-1 px-3 rounded-lg bg-slate-100 dark:bg-white/5 text-[#2EC4B6] hover:bg-[#2EC4B6] hover:text-white transition-all"
                            >
                                {text}
                            </button>
                        ))}
                ...
                <form
                    ...
                    className="relative flex items-center gap-2 bg-slate-100 dark:bg-white/5 p-1.5 rounded-xl border border-transparent focus-within:border-[#2EC4B6]/50 transition-all shadow-inner"
                >
                    <input
                        value={input}
                    ...
                    <button
                        type="submit"
                        className="bg-[#2EC4B6] text-white p-2.5 rounded-lg shadow-lg hover:shadow-[#2EC4B6]/50 transition-all"
                    >
                        <IoSend size={16} />
                    </button>
                </form>
