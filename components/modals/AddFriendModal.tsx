...
export default function AddFriendModal({ isOpen, onClose, onSendByNickname }: AddFriendModalProps) {
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : t('friends.errSendRequest'));
        } finally {
            setIsSending(false);
        }
    };
                        ...
                        <div className="p-8 md:p-10">
                            <div className="flex justify-between items-start mb-8">
                                <div>
                                    <h2 className="text-3xl font-black text-gray-900 dark:text-gray-100 tracking-tight">
                                        {t('addFriend.title1')} <span className="text-brand">{t('addFriend.title2')}</span>
                                    </h2>
                            ...
                            >
                                <span className="relative z-10 flex items-center justify-center gap-2">
                                    {isSending ? t('contacts.sending') : t('addFriend.send')}
                                </span>
