    ...
    return (
        <div className="relative min-h-dvh lg:h-dvh w-full bg-white dark:bg-[#0a0a0a] lg:bg-[#010606] lg:dark:bg-[#010606] overflow-x-hidden lg:overflow-hidden font-sans flex flex-col lg:block">
            ...
            <div className="absolute inset-0 w-full h-full z-0 hidden lg:block overflow-hidden bg-[#010606]">
                ...
                />
                <motion.div
                    animate={{ scale: [1.2, 1, 1.3, 1.2], x: ['0%', '20%', '-15%', '0%'], y: ['0%', '-25%', '15%', '0%'], rotate: [0, 180, 360] }}
                    transition={{ duration: 35, repeat: Infinity, ease: "linear" }}
                    className="absolute top-[15%] right-[25%] w-[45vw] h-[45vw] max-w-[700px] max-h-[700px] bg-[conic-gradient(from_90deg,#00F2FE,#168a7f,transparent,#00F2FE)] rounded-[50%_50%_40%_60%] mix-blend-color-dodge filter blur-[110px] opacity-50 pointer-events-none"
                />
            ...
            <div className="relative z-20 w-full h-full flex flex-col lg:flex-row">
                ...
                <div className="w-full lg:flex-1 min-h-[50vh] lg:min-h-full flex flex-col items-center justify-center p-8 lg:p-12 bg-[#010606] lg:bg-transparent relative overflow-hidden lg:overflow-visible">
                    ...
                    <div className="absolute inset-0 block lg:hidden pointer-events-none z-0 overflow-hidden bg-[#010606]">
                        <div className="absolute inset-0 bg-[linear-gradient(45deg,#00F2FE_0%,#168a7f_25%,#0F52BA_50%,#00F2FE_75%,#168a7f_100%)] bg-size-[400%_400%] animate-[gradient-xy_15s_ease_infinite] mix-blend-multiply opacity-[0.35]"></div>

                        <div className="absolute inset-0 opacity-[0.04] mix-blend-overlay" style={{ backgroundImage: 'url("data:image/svg+xml,%3Csvg viewBox=%220 0 200 200%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cfilter id=%22noiseFilter%22%3E%3CfeTurbulence type=%22fractalNoise%22 baseFrequency=%220.85%22 numOctaves=%223%22 stitchTiles=%22stitch%22/%3E%3C/filter%3E%3Crect width=%22100%25%22 height=%22100%25%22 filter=%22url(%23noiseFilter)%22/%3E%3C/svg%3E")' }}></div>

                        <motion.div
                            animate={{ scale: [1, 1.3, 1], rotate: [0, 180, 360], x: ['0%', '-15%', '0%'], y: ['0%', '15%', '0%'] }}
                            transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
                            className="absolute -top-[20%] -right-[30%] w-[160vw] h-[160vw] bg-[conic-gradient(from_0deg,#168a7f,#00F2FE,#020808,#168a7f)] rounded-[40%_60%_70%_30%] filter blur-[100px] opacity-60 pointer-events-none"
                        />
                        <motion.div
                            animate={{ scale: [1, 1.4, 1], rotate: [360, 180, 0], x: ['0%', '20%', '0%'], y: ['0%', '-15%', '0%'] }}
                            transition={{ duration: 30, repeat: Infinity, ease: "linear" }}
                            className="absolute bottom-[-15%] left-[-30%] w-[140vw] h-[140vw] bg-[conic-gradient(from_180deg,#a7ffeb,#00F2FE,#0F52BA,#a7ffeb)] rounded-[60%_40%_30%_70%] mix-blend-screen filter blur-[110px] opacity-40 pointer-events-none"
                        />

                        <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.025)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.025)_1px,transparent_1px)] bg-[size:40px_40px][mask-image:radial-gradient(ellipse_100%_100%_at_50%_50%,#000_15%,transparent_100%)] pointer-events-none" />
                    </div>
